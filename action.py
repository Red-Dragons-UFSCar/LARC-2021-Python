...
def shoot(robot, ball, leftSide=True, friends=[], enemys=[]):
    if leftSide:
        arrivalTheta = arctan2(
            90 - ball.yPos, 235 - ball.xPos
        )  # ? Angle between the ball and point (150,65)
    else:
        arrivalTheta = arctan2(
            90 - ball.yPos, 15 - ball.xPos
        )  # ? Angle between the ball and point (0,65)
...
def shoot_penalty(robot, ball, leftSide=True, friends=[], enemys=[]):
    if leftSide:
        arrivalTheta = arctan2(
            52 - ball.yPos, 235 - ball.xPos
        )  # ? Angle between the ball and point (150,65)
    else:
        arrivalTheta = arctan2(
            52 - ball.yPos, 15 - ball.xPos
        )  # ? Angle between the ball and point (0,65)
    # robot.target.update(ball.xPos,ball.yPos,0)
    robot.target.update(ball.xPos, ball.yPos, arrivalTheta)
...
def defenderSpin(
    ...
    if friend1 is None and friend2 is None:  # No friends to avoid
        v, w = univecController(
            robot, robot.target, avoidObst=False, n=16, d=2
        ...
        v, w = univecController(
            robot, robot.target, True, robot.obst, n=4, d=4, doubleFace=True
        )
    ...
    if (
        ...
        if flagVelocity:
            dy = (
                tan(robot.theta) * dx + robot.yPos
            )  # Calculate the height of the goal arrival
            if dy > 85 and dy < 95:
                if robot.index < 6:
                    robot.simSetVel2(
                        70 * robot.face, 70 * robot.face
                    )  # Send the velocity of right and left wheel
...
def screenOutBall(
    ...
    if robot.contStopped > 60:
        ...
        if friend1 is None and friend2 is None:  # ? No friends to avoid
            v, w = univecController(
                robot, robot.target, avoidObst=False, stopWhenArrive=True
            )
        else:  # ? Both friends to avoid
            robot.obst.update(robot, friend1, friend2)
            v, w = univecController(
                robot, robot.target, True, robot.obst, stopWhenArrive=True
            )
...
def directGoal(
    ...
    if robot.flagDirectGoal:
        ...
        if robot.dist(ball) < 10 and (
            robot.theta < (arrivalTheta + pi / 18)
            and (robot.theta > arrivalTheta - pi / 18)
        ):
...
def follower(
    ...
    """
    dist = sqrt(
        (robot_follower.xPos - proj_x) ** 2 + (robot_follower.yPos - proj_y) ** 2
    )
    arrival_theta = arctan2(
        ball.yPos - robot_follower.yPos, ball.xPos - robot_follower.xPos
    )
    robot_follower.target.update(proj_x, proj_y, arrival_theta)

    if (
        dist < 10
    ):  # Check if the robot is close to the projected point and stops the robot
        stop(robot_follower)
    else:
        # No friends to avoid
        if (
            robot0 is None
            and robot_enemy_0 is None
            and robot_enemy_1 is None
            and robot_enemy_2 is None
        ):
            v, w = univecController(
                robot_follower, robot_follower.target, avoidObst=False, n=16, d=2
            )
        else:  # Both friends to avoid
            robot_follower.obst.update2(
                robot_follower,
                ball,
                robot0,
                robot_leader,
...
def triple_leaderSelector(robot1, robot2, robot3, ball):
    ...
    if dist1 < dist2 and dist1 < dist3:  # Strategy if robot 2 is closer to the ball
        if (
            robot1.isLeader is None
            and robot2.isLeader is None
            and robot3.isLeader is None
        ):
            robot1.isLeader = True
            robot2.isLeader = False
...
def followLeader(
    ...
    if robot2.isLeader:
        if not robot1.teamYellow:
            if ball.xPos < 40 and (
                130 > ball.yPos > 50
            ):  # If ball is in defence side the robot 2 do the screen out, and the robot 1 follow his moves
                # print('hmmmm')
                if robot1.xPos < 30:
                    screenOutBall(
                        robot2,
                        robot2,
                        55,
                        leftSide=not robot2.teamYellow,
                        upperLim=120,
                        lowerLim=10,
                    )
                else:
                ...
                defenderSpin(
                    robot2,
                    ball,
                    left_side=not robot2.teamYellow,
                    friend1=robot0,
                    friend2=robot0,
                    enemy1=robot_enemy_0,
                    enemy2=robot_enemy_1,
                    enemy3=robot_enemy_2,
                    enemy4=robot_enemy_3,
                    enemy5=robot_enemy_4,
                )
                """
                ...
                if robot1.dist(ball) < 40:
                    if robot2.xPos > 195 and (100 > robot2.yPos > 40):
                        ...
                        defenderSpin(
                            robot1,
                            ball,
                            left_side=not robot1.teamYellow,
                            friend1=robot0,
                            friend2=robot2,
                            enemy1=robot_enemy_0,
                            enemy2=robot_enemy_1,
            ...
            if ball.xPos < 35 and (120 > ball.yPos > 50):
                if robot1.xPos < 35:
                    screenOutBall(
                        ...
                        robot1,
                        55,
                        leftSide=not robot1.teamYellow,
                        upperLim=120,
                        lowerLim=10,
                    )
                else:
...
def defenderPenalty(
    ...
    if leftSide:
        arrivalTheta = arctan2(
            ball.yPos - 90, ball.xPos - 15
        )  # ? Angle between the ball and point (150,65)
    else:
        arrivalTheta = arctan2(
            ball.yPos - 90, ball.xPos - 235
        )  # ? Angle between the ball and point (0,65)
    # robot.target.update(ball.xPos,ball.yPos,0)
    robot.target.update(ball.xPos, ball.yPos, arrivalTheta)
...
def atacante_lider(
    ...
    if enemy1 is None:
        v, w = univecController(
            robot, robot.target, avoidObst=False, stopWhenArrive=stop
        )
    else:
        obst = [friend, enemy1, enemy2, enemy3, enemy4, enemy5]
        robot.obst.update(robot, obst)
        v, w = univecController(
            robot, robot.target, avoidObst=True, obst=robot.obst, stopWhenArrive=stop
        )
...
def atacante_idle(robot, up, move=False, ball=None):
    ...
    robot.target.update(xPos, yPos, theta)
    v, w = univecController(
        robot, robot.target, avoidObst=False, stopWhenArrive=True, doubleFace=True
    )
    robot.simSetVel(v, w)
...
def defesa_atacantes(
    ...
    if (not robot0.teamYellow) and (
        ball.xPos > 70 or ball.yPos < 50 or ball.yPos > 130
    ):
        leaderSelector(robot3, robot4, ball)
...
def defenderWall(robot1, robot2, ball, leftSide=True):
    ...
    xtarget1, ytarget1, xtarget2, ytarget2 = circumferencePointProjection(
        robotTheta1, robotTheta2, ballTheta, raio, xgoal, 90
    )
    ...
    v, w = univecController(
        robot1, robot1.target, avoidObst=False, doubleFace=True, stopWhenArrive=True
    )
    robot1.simSetVel(v * 1.2, w * 1.2)
    robot2.target.update(xtarget2, ytarget2, arrivalTheta2)
    v, w = univecController(
        robot2, robot2.target, avoidObst=False, doubleFace=True, stopWhenArrive=True
    )
...
def defenderWallSolo(robot, ball, leftSide=True):
    ...
    v, w = univecController(
        robot, robot.target, avoidObst=False, doubleFace=True, stopWhenArrive=True
    )
...
def cruzamento(ball, robot2, robot3, alvo):
    ...
    v, w = univecController(
        robot3, robot3.target, avoidObst=False, doubleFace=True, stopWhenArrive=True
    )
...
def breakWall(
    ...
    arrivalTheta = 90
    robot.obst.update2(
        robot, ball, friend1, friend2, enemy1, enemy2, enemy3, enemy4, enemy5
    )

    robot.target.update(xtarget, ytarget, arrivalTheta)
    v, w = univecController(
        robot, robot.target, True, robot.obst, doubleFace=True, stopWhenArrive=True
    )
    robot.simSetVel(v, w)
...
def screenOutBall_diagonal(robot, ball, leftSide=True, friend1=None, friend2=None):
    ...
    if ball.yPos < midField_y:
        if leftSide:
            ...
            yPoint = midField_y - (
                (midField_y - lowerLim_y)
                * (upperLim_x - xPoint)
                / abs(upperLim_x - lowerLim_x)
            )
            if robot.xPos <= xPos:
                # arrivalTheta=arctan2((lowerLim_y-midField_y), (upperLim_x - lowerLim_x))
                arrivalTheta = arctan2(
                    (midField_y - lowerLim_y), (upperLim_x - lowerLim_x)
                )
            else:
                # arrivalTheta=arctan2((lowerLim_y-midField_y), (upperLim_x - lowerLim_x)) + pi
                arrivalTheta = (
                    arctan2((midField_y - 50), (upperLim_x - lowerLim_x)) + pi
                )
            ...
            yPoint = lowerLim_y + (
                (midField_y - lowerLim_y)
                * (xPoint - upperLim_x)
                / (lowerLim_x - upperLim_x)
            )
            if robot.xPos <= xPos:
                # Fazer para a borda
                arrivalTheta = (
                    arctan2((midField_y - lowerLim_y), (lowerLim_x - upperLim_x)) + pi
                )
                arrivalTheta = arctan2(sin(arrivalTheta), cos(arrivalTheta))
                print("aa")
            else:
                # Fazer para a borda
                arrivalTheta = arctan2(
                    (midField_y - lowerLim_y), (lowerLim_x - upperLim_x)
                )
        ...
        if leftSide:
            ...
            if robot.xPos <= xPos:
                # Fazer para a borda
                arrivalTheta = (
                    arctan2((midField_y - upperLim_y), (lowerLim_x - upperLim_x)) + pi
                )
            else:
                # Fazer para a borda
                arrivalTheta = arctan2(
                    (midField_y - upperLim_y), (lowerLim_x - upperLim_x)
                )
            robot.target.update(xPoint, yPoint, arrivalTheta)
